"""BookingRulesEntityValidator: a prior-day rule needs a last day to book.

One of a pair with missing_prior_notice_last_time, both gated on PRIORDAY and both
emitted for the same row when neither field is present.
"""
...
def check(row: dict, ctx: Context) -> Iterator[Notice]:
    if row.get("booking_type") != booking.PRIORDAY or booking.has(row, booking.LAST_DAY):
        return
